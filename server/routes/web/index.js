...
module.exports = app => {
  ...
  // const Article = require('../../models/Article')因为模型引入到了moogose里所以引用模型可以使用上面的方式

  //导入新闻数据
  router.get('/news/init', async (req, res) => {//初始化新闻接口
    const parent = await Category.findOne({ name: '新闻分类' })//表示取出新闻分类
    const cats = await Category.find().where({
      parent: parent//表示从哪寻找
    }).lean()//lean表示取纯粹的json，js对象或数组
    const newsTitles = ["六城共庆王者荣耀五周岁，同心筑梦祈福", "营地看庆典，福利大放送！赢荣耀水晶、", "【王者共创荣耀盛典】海量精彩节目等你pick", "无限王者团携手宋茜 打破虚实演绎千面", "吃喝玩购样样省钱，百城万店狂撒优惠", "11月1日王者营地直播、游戏内电视台直播", "11月1日周年庆典活动奖励到账延迟说明", "10月29日全服不停机更新公告", "10月29日全服不停机更新公告", "10月28日体验服停机更新公告", "【美梦成真 抽内测皮肤】活动公告", "周年祈愿送永久英雄/皮肤", "王者共创，荣耀盛典，抽内测、赢永久、", "手Q用户专属】周年庆返场皮肤免单活动", "【微信用户专属】微信小程序“游戏礼品", "2020年秋冬赛事战令全新升级，终极奖", "2020年KPL秋季赛9月16日热血开赛，主场", "2020年KPL秋季赛热血来袭，线下售票9月", "2020年KGL秋季赛选手大名单公布，9月1", "2020年KPL秋季赛大名单公布"]
    const newsList = newsTitles.map(title => {
      const randomCats = cats.slice(0).sort((a, b) => Math.random() - 0.5)//打乱 slice(0)表示从cats复制一份以免影响原来的数组
      return {
        categories: randomCats.slice(0, 2),
        title: title
      }
    })
    await Article.deleteMany({})//这两条用于更新内容
  ...
  router.get('/news/list', async (req, res) => {
    ...
    const cats = await Category.aggregate([
      { $match: { parent: parent._id } },//条件查询 前一个表示字段 后一个表示上面查到的parcats 用match来排除一些数据
      {
        $lookup: {//查另外一个集合
          from: 'articles',//默认为模型名的小写复数 在哪里查找
          localField: '_id',//本地键
          foreignField: 'categories',//对应外地键
          as: 'newsList'//命名
        }
      },
      {
        $addFields: {//筛选 slice第一个参数为筛选的对象 第二个为筛选个数
          newsList: { $slice: ['$newsList', 5] }
        }
      }
    ])
    const subCats = cats.map(v => v._id)
    cats.unshift({//在前面添加热门分类
      name: '热门',
      newsList: await Article.find().where({
        categories: { $in: subCats }
      }).populate('categories').limit(5).lean()//populate关联查询
    })
    cats.map(cat => {
      cat.newsList.map(news => {
        news.categoryName = (cat.name === '热门') ? news.categories[0].name : cat.name//如果等于热门 显示原来的分类
        return news
      })
      return cat
    })
  ...
  router.get('/heroes/init', async (req, res) => {
    ...
    for (let cat of rawData) {//循环各个项目 in用在索引值的时候 k in ... of直接用于循环
      ...
      cat.heroes = cat.heroes.map(hero => {//此处hero为单个英雄
  ...
  router.get('/heroes/list', async (req, res) => {
    ...
    //聚合查询
    const cats = await Category.aggregate([
      { $match: { parent: parent._id } },//条件查询 前一个表示字段 后一个表示上面查到的parent 根据id查询 用match来排除一些数据
      {
        $lookup: {//查另外一个集合
          from: 'heroes',//默认为模型名的小写复数 在哪里查找
          localField: '_id',//本地键
          foreignField: 'categories',//对应外地键
          as: 'heroList'//命名
        }
    ...
    cats.unshift({//在前面添加热门分类
      name: '热门',
      heroList: await Hero.find().where({//hero为模型
        categories: { $in: subCats }
      }).limit(10).lean()
    })
    res.send(cats)
  ...
  router.get('/articles/:id', async (req, res) => {
    const data = await Article.findById(req.params.id).lean()//根据传入的id在模型中查找
  ...
  router.get('/heroes/:id', async(req, res)=> {
    const data = await Hero.findById(req.params.id).populate('categories items1 items2 partners.hero').lean()//转成纯粹的json数据 在加参数的时候会比较方便 populate关联查询 调出分类全部数据
    // categories items1 items2 在hero模型中直接用id关联(ref)了模型 partners为partners.hero
    res.send(data)
  })
